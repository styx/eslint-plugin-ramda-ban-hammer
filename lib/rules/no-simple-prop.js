...
module.exports = {
  ...
  create(context) {
    ...
    function isKeyMemberExpression(node) {
    ...
    return {
      CallExpression(node) {
        if (isSimpleRProp(node) && !isKeyMemberExpression(node)) {
          context.report({
            node,
            ...
            fix(fixer) {
              ...
              const key = sourceCode.getText(prop);
              return fixer.replaceText(node, `${reciver}[${key}]`);
            },
          });
        }
      },
