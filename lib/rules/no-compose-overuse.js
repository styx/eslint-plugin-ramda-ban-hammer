...
module.exports = {
  ...
  create(context) {
    function isComposeWithArg(node) {
      ...
      node.callee.callee.object.name === 'R' &&
      node.callee.callee.property.name === 'compose';
    }
    //----------------------------------------------------------------------
    // Public
    //----------------------------------------------------------------------
        ...
        if (isComposeWithArg(node)) {
          context.report({
            ...
            fix(fixer) {
              ...
              const argument = sourceCode.getText(node.arguments[0]);
              ...
              if (firstFuncCode.slice(-1) === ')') {
                result = firstFuncCode.replace(/\)$/, `, ${argument})`);
              } else {
                result = `${firstFuncCode}(${argument})`;
              }
